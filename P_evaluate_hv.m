function Metric = P_evaluate_hv(Operation,FunctionValue,RefPoint)
    ...
    switch Operation
        ...
        %超体积
        case 'HV'
%             RefPoint = max(RefPoint,[],1) + 1;    
%              RefPoint = max(FunctionValue,[],1) + 0.01*(max(FunctionValue,[],1)-min(FunctionValue,[],1));  
...
%             elseif M < 4
        ...
        %归一化的超体积
        case 'nHV'
%             Metric = P_evaluate_hv('HV',FunctionValue,RefPoint)/abs(prod(max(RefPoint,[],1)));
            Metric = P_evaluate_hv('HV',FunctionValue,RefPoint);
        case 'SM'
